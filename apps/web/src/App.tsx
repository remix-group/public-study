      ...
      {screen === "welcome" && (
        <main className="student-home">
          ...
          <section className="today-plan">
            <div className="today-copy"><span className="today-badge"><Icon name="target"/> Siguiente acción recomendada</span><h2>{dashboard?.recommendedObjective?.objective ?? "Diagnóstico inicial"}</h2><p>{dashboard?.recommendedObjective?.description ?? "Responde una primera sesión para que podamos estimar tus fortalezas y vacíos."}</p>{dashboard?.recommendedObjective?.reason && <div className="recommendation-reason">Por qué: {dashboard.recommendedObjective.reason}</div>}<div className="plan-meta"><span><Icon name="clock"/> 5–10 minutos</span><span><Icon name="shield"/> Evidencia jurídica</span><span>{dashboard?.recommendedObjective?.questionCount ?? 10} preguntas disponibles</span></div><div className="mode-actions"><button className="button primary" onClick={() => openGuide(dashboard?.recommendedObjective?.objectiveId)}>Aprender <span>→</span></button><button className="button secondary" onClick={() => begin(dashboard?.recommendedObjective?.objectiveId, dashboard?.recommendedObjective?.action === "REVIEW" ? "REVIEW" : "PRACTICE")}>{dashboard?.recommendedObjective?.action === "REVIEW" ? "Repasar" : "Practicar"}</button></div></div>
